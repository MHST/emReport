...
if (!defined('NV_IS_EMREPORT_ADMIN'))
    die('Stop!!!');
$page_title = $lang_module['doctor_add'];
...
if ($nv_Request->isset_request('confirm', 'post'))
{
    ...
    $_user['cmnd'] = filter_text_input('cmnd', 'post', '', 1, 100);
    $_user['khoa'] = filter_text_input('khoa', 'post', '', 1, 100);
    $_user['trinhdo'] = filter_text_input('trinhdo', 'post', '', 1, 100);
    $_user['ngaycapbang'] = filter_text_input('ngaycapbang', 'post', '', 1, 10);
    $_user['mabenhvien'] = filter_text_input('mabenhvien', 'post', '', 1, 100);
    ...
    {
        $error = $lang_module['edit_error_email_exist'];
    } elseif (($check_pass = nv_check_valid_pass($_user['password1'], NV_UPASSMAX,
    NV_UPASSMIN)) != "")
    {
        $error = $check_pass;
    ...
    } else
    {
    	...
    	if (preg_match("/^([0-9]{1,2})\.([0-9]{1,2})\.([0-9]{4})$/", $_user['ngaycapbang'],
            $m))
        {
            $_user['ngaycapbang'] = mktime(0, 0, 0, $m[2], $m[1], $m[3]);
        } else
        {
            $_user['ngaycapbang'] = 0;
        }
        ...
        {
        	// them bac si
	        $query = "INSERT INTO `" . NV_PREFIXLANG . "_" . $module_data . "_bacsi` (`cmnd`, `ten`, `khoa`, `trinhdo`, `ngaycapbang`, `mabenhvien`) 
	        	VALUES ('". $_user['cmnd'] . "', '" . $_user['username'] . "', '" . $_user['khoa'] . "', '" . $_user['trinhdo'] . "', '" . $_user['ngaycapbang'] . "', '" . 
	        		$_user['mabenhvien'] . "')";
			$db->sql_query($query);
            ...
            if (isset($_FILES['photo']) and is_uploaded_file($_FILES['photo']['tmp_name']))
            {
                ...
                $upload = new upload(array('images'), $global_config['forbid_extensions'], $global_config['forbid_mimes'],
                    NV_UPLOAD_MAX_FILESIZE, 80, 80);
                $upload_info = $upload->save_file($_FILES['photo'], NV_UPLOADS_REAL_DIR . '/' .
                    $module_name, false);
                ...
                if (empty($upload_info['error']))
                {
                    ...
                    $sql = "UPDATE `" . NV_USERS_GLOBALTABLE . "` SET `photo`=" . $db->dbescape($file_name) .
                        " WHERE `userid`=" . $userid;
                    $db->sql_query($sql);
                }
            }
            if (!empty($_user['in_groups']))
            {
                ...
                {
                    $query = "SELECT `users` FROM `" . NV_GROUPS_GLOBALTABLE . "` WHERE `group_id`=" .
                        $group_id_i;
                    ...
                    if ($numrows)
                    {
                        ...
                        $sql = "UPDATE `" . NV_GROUPS_GLOBALTABLE . "` SET `users`=" . $db->
                            dbescape_string($users) . " WHERE `group_id`=" . $group_id_i;
                        $db->sql_query($sql);
                    }
                }
            }
...
{
    $_user['username'] = $_user['cmnd'] = $_user['khoa'] = $_user['trinhdo'] = $_user['ngaycapbang'] = $_user['mabenhvien'] = 
    $_user['email'] = $_user['password1'] = $_user['password2'] =
        $_user['question'] = $_user['answer'] = "";
    $_user['full_name'] = $_user['gender'] = $_user['website'] = $_user['location'] =
        $_user['yim'] = $_user['telephone'] = "";
...
if (!empty($groups_list))
{
    foreach ($groups_list as $group_id => $grtl)
    {
        $groups[] = array(
            'id' => $group_id,
            'title' => $grtl,
            'checked' => (!empty($_user['in_groups']) and in_array($group_id, $_user['in_groups'])) ?
                " checked=\"checked\"" : "");
    }
}
$xtpl = new XTemplate("doctor_add.tpl", NV_ROOTDIR . "/themes/" . $global_config['module_theme'] .
    "/modules/" . $module_file);
$xtpl->assign('LANG', $lang_module);
$xtpl->assign('DATA', $_user);
$xtpl->assign('FORM_ACTION', NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE .
    "=" . $module_name . "&amp;" . NV_OP_VARIABLE . "=doctor_add");
