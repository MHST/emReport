...
if ($nv_Request->isset_request('confirm', 'post'))
{
    ...
    } else
    {
        ...
        $sql = "INSERT INTO `" . NV_USERS_GLOBALTABLE . "` (
        	`username`, `md5username`, `password`, `email`, `full_name`, `gender`, `birthday`, `regdate`, `location`,
        	`remember`, `active`)        
        	VALUES(" . $db->dbescape($_user['username']) . "," . $db->dbescape(md5($_user['username'])) .
...
{
    $_user['username'] = $_user['cmnd'] = $_user['email'] = "";
...
$xtpl = new XTemplate("user_add.tpl", NV_ROOTDIR . "/themes/" . $global_config ['module_theme'] . "/modules/" . $module_name);
